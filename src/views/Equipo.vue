<template>
  <div class="about">
    <div class="hero is-danger">
      <div class="hero-body">
        <div class="container">
          <h1 class="title is-size-1">¿Cómo funciona?</h1>
        </div>
      </div>
    </div>
    <div class="container">
      <p class="org-description is-size-4">
        Hola! nos complace saber que deseas conocer sobre nuestro proyecto.
        Este está pensado en ayudar con la activación económica y hacer visibles los servicios de parqueadero y sus adicionales.
        <br>
        Lo que debes hacer es registrarte y usar nuestro directorio
        <br>
        El paqueadero es un sistema que posee una plataforma Movil. Dicho dispositivo puede agendar su estacionamiento de los vehiculos y asi evitar las filas para hacer el ingreso a la plataforma. </p>
    </div><br>
    <div class="hero is-danger">
      <div class="hero-body">
        <div class="container">
          <h1 class="title is-size-1">¿Que Ofrecemos?</h1>
        </div>
      </div>
    </div>
    <div class="container">
      <p class="org-description is-size-4">
        Ademas de ofrecer servicio de parqueo, tambien ofrecemos servicio de lavanderia para automoviles, ofrecemos una plataforma para servicio automatizado de parqueo Online.</p>
    </div><br>
    <div class="hero is-danger">
      <div class="hero-body">
        <div class="container">
          <h1 class="title is-size-1">¿Donde estamos Ubicados?</h1>
        </div>
      </div>
    </div>
    <div class="container">
      <p class="org-description is-size-4">
        Estamos Ubicados en Bogota.</p>
    </div><br><br><br> 
  </div>
</template>
<style lang="scss" scoped>
  .org-description {
    margin-top: 50px;
  }
</style>
